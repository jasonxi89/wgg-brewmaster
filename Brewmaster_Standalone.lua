...
local function Bootstrap(attempt)
    ...
    local function CoreRotation(target)
        ...
        if comboState == "waiting_bof" then
            -- Simple approach: try BoF, if it works great, if not just wait
            -- Has Sal talent → we KNOW BoF CD will be reset, just wait for GCD/latency
            ...
            if StateCache.hasSalTalent then
                -- Trust the talent: BoF CD WILL be reset. Just wait.
                -- The 1.5s timeout is the only escape.
                return false
            else
                -- No Sal: check if BoF is truly on CD
                local bofCD = Spells.BreathOfFire and Spells.BreathOfFire.cd or 999
                if bofCD > 0 then
                    -- BoF on real CD, no Sal to reset → abandon
                    comboState = "idle"
                    comboStartTime = 0
                else
                    -- BoF CD=0 but Castable failed (GCD/facing) → wait
                    return false
                end
            end
        end
    ...
    -- Register UNIT_SPELLCAST_SUCCEEDED for confirmed spell tracking
    -- (COMBAT_LOG_EVENT_UNFILTERED doesn't work in WGG environment)
    local confirmFrame = CreateFrame("Frame")
    confirmFrame:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED")
    confirmFrame:SetScript("OnEvent", function(_, _, unit, _, spellID)
        if unit ~= "player" or not running then return end
        local spellName = C_Spell and C_Spell.GetSpellName and C_Spell.GetSpellName(spellID) or tostring(spellID)
        Logger:Log("spell_confirmed", spellName, {
            spellId = spellID,
            spellName = spellName,
        }, warden.target)
    end)
